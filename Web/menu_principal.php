...
session_start();
$conn = new mysqli("localhost","root","","bd_magaly");
$id = $_SESSION['id'];
$info = mysqli_query($conn, "SELECT * FROM usuarios WHERE id_usuario='$id'");
$info_recientes = mysqli_query($conn, "SELECT * FROM pelicula WHERE year(fechaEstreno) = 2022");
$info_joven = mysqli_query($conn, "SELECT * FROM pelicula WHERE categoria = 'Jovenes'");
$info2 = mysqli_fetch_array($info);
$perfil = $_GET['user'];
$i = 0;
$ii = 0;
$j = 0;
if($perfil == $info2['nombre_perfil_usuario']){
  $nombre = $info2['nombre_perfil_usuario'];
  $imgperfil = $info2['img_perfil1_usuario'];
}else{
  $nombre = $info2['nombre_perfil_usuario2'];
  $imgperfil = $info2['img_perfil2_usuario'];
}

$_SESSION['perfil'] = $nombre;
$_SESSION['imgperfil'] = $imgperfil;

$info3 = mysqli_query($conn, "SELECT * FROM pelicula order by titulo asc limit 3");
$totalcarrouselgrande = mysqli_num_rows($info3);

$carrouselinferior = mysqli_query($conn, "SELECT * FROM pelicula order by id desc limit 24");
?> -->
...
<head>
    ...
    <title>FIC+ | <?php echo $_SESSION['perfil'];?></title>
</head>
<body>
<div class="offcanvas offcanvas-start bg-dark text-bg-white" style="width:18vw" tabindex="-1" id="offcanvas" data-bs-keyboard="false" data-bs-backdrop="false">
        <div class="offcanvas-header">
            <img src="../img/logo01v3.png" width="60" height="50" alt="Imagen Logo" class="logo_central offcanvas-title">
            ...
            <ul class="nav nav-pills flex-column mb-sm-auto mb-0 align-items-start" id="menu">
                <li class="dropdown">
                    <a href="#" class="d-flex align-items-center text-white text-decoration-none dropdown-toggle mb-2 px-3" id="dropdownUser1" data-bs-toggle="dropdown" aria-expanded="false">
                        <img src="<?php echo $_SESSION['imgperfil']; ?>" alt="hugenerd" width="30" height="30" class="rounded-circle">
                        <span class="d-none d-sm-inline mx-1"><?php echo $_SESSION['perfil'] ?></span>
                    </a>
                ...
                <li class="nav-item">
                    <a style ="Color: White;" href="menu_principal.php?user=<?php echo $nombre?>" class="nav-link text-truncate">
    ...
    <div style="height: 75vh;"id="carouselExampleCaptions" class="carousel slide" data-bs-ride="false">
        <div class="carousel-indicators">
          <?php for($contador = 0;$contador<$totalcarrouselgrande;$contador++){ ?>
            <button type="button" data-bs-target="#carouselExampleCaptions" 
            data-bs-slide-to="<?php echo $contador;?>"aria-current="true" class="<?php if($contador==0){echo "active";}else{echo "";}?>" aria-label="Slide <?php echo $contador;?>"></button>
          <?php } ?>
           
        </div>
        <div class="carousel-inner">
          <!--Esta linea de codigo Lee lo que se obtuvo de la base de datos-->
           <?php while($info4 = mysqli_fetch_array($info3)){ ?>
              <div class="carousel-item active">
                <a href="../Paginas/peliculas.php?pelicula=<?php echo $info4['id'];?>"><img src="<?php echo $info4['foto_pelicula'];?>" class="d-block" alt="<?php echo $info4['titulo'];?>_img"></a>
                <div href="../Paginas/peliculas.php?<?php echo $info4['id'];?>" class="carousel-caption d-none d-md-block">
                  <h1>Bienvenido de Vuelta <?php echo $_SESSION['perfil']; ?> </h1>
                  <h5><?php echo $info4['titulo'];?></h5>
                  <p><?php echo $info4['decripcion'];?></p>
                </div>
              </div>
            <?php } ?>
          </div>
